...
# def search_by_image():
#     """
#     Search product by uploading an image
#     Body: form-data ('file': image_file)
#     """
...
#         # 2. Tạo vector từ ảnh upload
#         query_vector = clip_model.encode(image, convert_to_numpy=True, normalize_embeddings=True).tolist()
        
#         # 3. Tìm kiếm trong Postgres bằng pgvector
#         # Lưu ý: service.db là instance của class Database bạn đã viết
#         # Giả sử limit 10 kết quả
#         results = service.db.query("""
#             SELECT 
#                 pf.product_id,
#                 pf.current_price,
#                 pf.avg_rating,
#                 1 - (pf.image_embedding <=> %s::vector) as similarity
#             FROM product_features pf
#             WHERE pf.image_embedding IS NOT NULL
#             ORDER BY pf.image_embedding <=> %s::vector ASC
#             LIMIT 10
...
#             products.append({
#                 'product_id': row['product_id'],
#                 'similarity': float(row['similarity']), # Điểm càng gần 1 càng giống
