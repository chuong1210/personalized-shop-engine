...
def generate_users(n_users=500):
    """
    Tạo danh sách user kết hợp: User thật từ MySQL + User giả (nếu thiếu)
    """
    # 1. Lấy user thật
    real_users = fetch_real_users_from_mysql(limit=n_users)
    
    # 2. Nếu chưa đủ n_users, tạo thêm user giả
    if len(real_users) < n_users:
        needed = n_users - len(real_users)
        print(f"   Not enough real users. Generating {needed} fake users...")
        fake_users = [str(uuid.uuid4()) for _ in range(needed)]
        final_list = real_users + fake_users
    else:
        # Nếu thừa thì lấy ngẫu nhiên cho đúng số lượng yêu cầu
        final_list = random.sample(real_users, n_users)
        
    # Shuffle để trộn lẫn
    random.shuffle(final_list)
    return final_list
def fetch_real_users_from_mysql(limit=1000):
    """
    Lấy danh sách user_id thực tế từ bảng product_comment (hoặc bảng order nếu có).
    Để đảm bảo khi test API bằng user thật, họ đã có dữ liệu train.
    """
    conn = connect_mysql_order()
    real_users = []
    try:
        # Lấy user_id từ bảng comment (những người tích cực tương tác)
        # Bạn có thể UNION thêm từ bảng orders nếu muốn nhiều user hơn
        sql = """
        ...
        with conn.cursor() as cur:
            ...
            rows = cur.fetchall()
            # rows trả về dạng list các tuple, ví dụ [('user1',), ('user2',)]
            # Cần flatten thành list ['user1', 'user2']
            real_users = [str(row[0]) for row in rows]
            
        print(f"📊 Found {len(real_users)} real users from MySQL.")
        return real_users
    except Exception as e:
        print(f"⚠️ Error fetching real users: {e}")
        return []
    finally:
        conn.close()
